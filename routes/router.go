...
func SetupRoutes(app *fiber.App) {
	...
	user.Get("/:id", middleware.AdminOnly(), handlers.GetUser)
	user.Post("/register", handlers.CreateUser)
	user.Patch("/:id", middleware.AdminOnly(), handlers.UpdateUser)
	user.Delete("/:id", middleware.AdminOnly(), handlers.DeleteUser)
	...
	book.Post("/", middleware.AdminOnly(), handlers.CreateBook)      // теперь только админы могут получить
	book.Delete("/:id", middleware.AdminOnly(), handlers.DeleteBook) // доступ к эти рутам
